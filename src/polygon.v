...
(* Кажется, что корректность порядка -- отсутствие противоречий типа 1 < 4 <  2 && 2 < 5 < 1 
    А также, что в нашем порядке вершина, которая явно идет раньше по графу не идет раньше по порядку *)

(* Show для частичного порядка *)
(* Частичный порядок только на поддеревьях, поэтому верхнее дерево выполняется раньше чем нижнее *)
    [4]; [3]; [5; 7]; [2; 3; 4; 3; 4]; [1; 5; 7; 2; 3; 4; 6; 5; 7; 6];
       [7; 2; 3; 4]; [6; 5]]